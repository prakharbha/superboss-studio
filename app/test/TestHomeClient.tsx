...
export default function TestHomeClient({ studiosData }: TestHomeClientProps) {
            ...
            {useCases.map((useCase, index) => (
              <AnimatedSection key={index} delay={index * 0.05}>
                <div className="bg-white p-6 rounded-lg text-center hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                  <Sparkles className="w-8 h-8 text-sb-black mx-auto mb-3" />
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <AnimatedSection>
              <div className="text-center mb-12">
                ...
                <div className="w-24 h-1 bg-sb-black mx-auto mb-8"></div>
                <p className="text-lg text-sb-grey max-w-2xl mx-auto">
                  Explore our versatile spaces designed for every creative vision
                </p>
              </div>
            </AnimatedSection>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {studiosData.slice(0, 6).map((studio, index) => (
                <AnimatedSection key={studio.id} delay={index * 0.1}>
                  <Link href={`/studios/${studio.slug}`}>
                    <div className="bg-white rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-2">
                      ...
                      <div className="p-6">
                        <h3 className="text-2xl font-bold text-sb-black mb-2">{studio.name}</h3>
                        <p className="text-sb-grey mb-4">{studio.size} {studio.unit}</p>
                        <p className="text-sb-grey text-sm mb-4 line-clamp-2">{studio.description}</p>
                        <div className="flex items-center text-sb-black font-semibold">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <AnimatedSection>
            ...
            <p className="text-xl text-white/80 mb-8">
              Book your studio space today and start creating
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <Link
                href="/book"
                className="px-10 py-5 bg-white text-sb-black text-xl font-bold rounded-lg hover:bg-sb-grey-light transition-all duration-300 hover:scale-110 flex items-center justify-center space-x-2"
              >
                <span>Book Now</span>
                <ArrowRight className="w-6 h-6" />
              </Link>
